...
class Level:
    def __init__(self,  curr_level, surface, create_overworld):
        ...
        self.level_data = levels[curr_level]
        self.level_content = self.level_data['content']
    ...
    def load_data(self):
        ...
        self.infont = path.join(self.img_folder, 'STAMPACT.ttf')
        self.ammofont = path.join(self.img_folder, 'ammo.ttf')
        self.countfont = path.join(self.img_folder, 'LEVIBRUSH.TTF')
        self.player_face = pg.image.load(path.join(self.img_folder, FACE_LOGO )).convert_alpha()
        self.title_font = path.join(self.img_folder, 'ZOMBIE.TTF')
        
        self.player_img = pg.image.load(path.join(self.img_folder, PLAYER_IMGS['pistol'] )).convert_alpha()
        self.bullet_images = {}
        self.gun_images = {}
        self.gun_images['pistol'] = pg.image.load(path.join(self.img_folder, 'pistol_show.png' )).convert_alpha()
        self.gun_images['shotgun'] = pg.image.load(path.join(self.img_folder, 'shotgun_show.png' )).convert_alpha()
        self.gun_images['rifle'] = pg.image.load(path.join(self.img_folder, 'ar_show.png' )).convert_alpha()
        self.ammo_img = pg.image.load(path.join(self.img_folder, AMMO_IMG )).convert_alpha()
        self.hazard_img = pg.image.load(path.join(self.img_folder, HAZARD_IMG )).convert_alpha()
        ...
        self.mob_img = pg.image.load(path.join(self.img_folder, ZOMBIE_IMG )).convert_alpha()
        self.wall_img = pg.image.load(path.join(self.img_folder, WALL_IMG )).convert_alpha()
        self.wall_img = pg.transform.scale(self.wall_img, (TILESIZE , TILESIZE))
        self.splat = pg.image.load(path.join(self.img_folder, 'splat green.png')).convert_alpha()
        self.splat = pg.transform.scale(self.splat, (64,64))
        self.go_img = pg.image.load(path.join(self.img_folder, GAME_OVER )).convert_alpha()
        self.won_img = pg.image.load(path.join(self.img_folder, SURVIVED_IMAGE)).convert_alpha()
    ...
    def new(self):
        ...
        self.map = TiledMap(path.join(self.map_folder, self.level_content))
        self.map_img = self.map.make_map()
        self.map_rect = self.map_img.get_rect()
        for wpn in WEAPONS:
            if wpn not in ['pistol']:
                GUN[wpn]['equipped'] = False
                GUN[wpn]['ammo'] = 0
            else:
        ...
        for tile_object in self.map.tmxdata.objects:
            ...
            if tile_object.name == 'zombie' and self.total_zombies < 2:
                self.total_zombies += 1
                co_ords.append((obj_center.x , obj_center.y))
        ...
        self.paused = False
        self.night = self.level_data['night']
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                ...
                if event.key == pg.K_2 and self.player.weapon != 'shotgun' and GUN['shotgun']['equipped']:
                    self.effects_sound['gun_switch'].play()   
                    self.player_img = pg.image.load(path.join(self.img_folder, PLAYER_IMGS['shotgun'] )).convert_alpha()   
                    self.player.weapon = 'shotgun'        
                if event.key == pg.K_3 and self.player.weapon != 'rifle' and GUN['rifle']['equipped']:
                    self.effects_sound['gun_switch'].play()      
                    self.player_img = pg.image.load(path.join(self.img_folder, PLAYER_IMGS['rifle'] )).convert_alpha()
    ...
    def show_go_screen(self):
        ...
        if self.total_zombies > 0:
            ...
            self.draw_text("Zombies ate your BRAIN!" , self.countfont , 85 , RED ,
                            WIDTH/2, 5 , align = 'n') 
            self.draw_text("Press Enter to Return!" , self.infont , 20 , WHITE ,
                            WIDTH/2, HEIGHT - 50 , align = 's')                
        else:
            self.survived = True
            self.effects_sound['victory'].play()
            ...
            self.draw_text("You Survived the Apocalypse!" , self.countfont , 80 , GREEN ,
                            WIDTH/2, 5 , align = 'n') 
            self.draw_text("'Press Enter' to Continue!" , self.infont , 15 , WHITE ,
        ...
        while waiting:
            ...
            for event in pg.event.get():
                ...
                if event.type == pg.KEYUP:
                    if event.key == pg.K_RETURN and not self.survived:
                        waiting = False
                        self.create_overworld(self.curr_level, 0)
                    if event.key == pg.K_RETURN and self.survived: 
                        waiting = False
